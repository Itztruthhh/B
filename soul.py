...
bot = telebot.TeleBot('7613580419:AAEO8gACP3j32g1PvticnL9dG1kdQtf5-ao')
...
def start_attack_reply(message, target, port, time):
    ...
    # Dictionary to store the last time each user ran the /vip command
bgmi_cooldown = {}
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            response = "Usage: /vip <target> <port> <time>"
    else:
...
def welcome_start(message):
    ...
    response = f'''🔥vip ke LODE pe aapka swagat hai, {user_name}! Sabse acche se bgmi ki maa behen yahi hack karta hai. Kharidne ke liye Kira se sampark karein.
